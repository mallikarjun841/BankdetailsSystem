...
const connection = mysql.createConnection({
  ...
  database: "useerdata",
  user: "root",
  authentication_string: "*2D03E8B867F214922669F08E0F1DCDB58F3FDFAB",
...
app.post("/userregister/", async (request, response) => {
  ...
  connection.query(
    "select * from useerdata.register where username=?;",
    username,
    function (error, results, fields) {
      ...
      if (results.length === 0) {
        ...
        connection.query(
          "INSERT INTO useerdata.register SET ?",
          { username: username, email: email, password: encrpt },
...
const authenticatetoken = (request, response, next) => {
  ...
  if (token === undefined) {
    ...
    const verifytoken = jwttoken.verify(
      token,
      "thissecretkey",
      async (err, user) => {
        if (err) {
...
app.post("/userdetails/", async (request, response) => {
  ...
  connection.query(
    `select * from useerdata.register where email='${email}';`,
    function (error, result, fields) {
      ...
      if (result) {
        ...
        if (comparepassword) {
          jwttoken.sign(payload, "thisisscretkey", (error, token) => {
            if (error) throw error;
        ...
        response.status(400);
        response.send({ error: "email not exits" });
      }
    }
  );
});
...
app.post("/userlogin/", (request, response) => {
  connection.query(
    "select * from user where usernaem=?",
    username,
